...
> **Important:** GitHub runs `release`-triggered workflows from the **default
> branch**. `publish.yml` must be committed to `main` (not just a feature
> branch) or publishing a Release does nothing — no run appears under Actions.
> The workflow also supports `workflow_dispatch`, so you can trigger the first
> publish manually from the Actions tab once it's on `main`.
...
`fastmcp-guard` does not exist on PyPI yet, so use the **pending publisher**
form — it pre-authorizes the very first publish, which creates the project:

1. Log in to PyPI and go to
...
2. Under **Add a new pending publisher → GitHub**, enter:
   - **PyPI Project Name:** `fastmcp-guard`
   - **Owner:** `rohanprichard`
...
> Once the project exists (after the first publish), you can manage the same
> publisher at `https://pypi.org/manage/project/fastmcp-guard/settings/publishing/`.
...
[https://test.pypi.org](https://test.pypi.org) for this repo, then temporarily point the publish step at
it:
